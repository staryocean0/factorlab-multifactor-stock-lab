...
**状态与负责人**：待用户转交本地大模型；本地尚未执行、尚无反馈。云端负责本次任务定义、已有源码定位与反馈复核，本地负责缺失数据环境中的实际检查。本次用户明确要求云端—本地协作，故启用 AGENTS.md 两块协议；不影响其他普通本地任务。
...
**本地反馈（待填写）**：请把小型 `local_feedback.md` 的位置、实际源码/数据版本、执行范围、成功/失败/未执行事项、最小返工方案和必要产物链接写在此处；保留原云端交接文字。可以在本任务下追加多次反馈，不重复建同一任务。大数据和大产物留本地；公开仓库只提交许可允许的小型报告/代码，不含凭据或受限数据。不能回写时由用户转交文档。
