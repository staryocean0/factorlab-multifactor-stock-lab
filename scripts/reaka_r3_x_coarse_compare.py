#!/usr/bin/env python3
"""Fit only E on the already accepted R3 TIMEISO run; never refit F/H."""
...
def _bootstrap() -> None:
    ...
    for name in ("reaka_r3_condition_views", "reaka_r3_time_isolation", "reaka_r3_time_isolated_runner",
                 "reaka_r3_x_decomposition", "reaka_r3_x_coarse_runner"):
...
def main() -> int:
    ...
    parser.add_argument("--reload-e-worker", type=Path)
    args = parser.parse_args()
    try:
        if args.reload_e_worker:
            s = read_spec(args.reload_e_worker)
            out = coarse.reload_e_worker(
                Path(s["store_root"]), Path(s["checkpoint_root"]), Path(s["normalizer_path"]),
                Path(s["candidate_path"]), int(s["seed"]), Path(s["indices_path"]), Path(s["output_path"]),
                str(s.get("device_name", "cpu")),
            )
            print(json.dumps(out, ensure_ascii=False)); return 0
        if not args.spec:
            raise ValueError("--spec required")
        s = read_spec(args.spec)
        if s.get("schema_id") != "factorlab.r3_x_coarse_run_spec@1.0":
            raise ValueError("wrong schema")
        result = coarse.run(
            Path(s["accepted_timeiso_run_root"]).resolve(), Path(s["output_root"]).resolve(), Path(__file__).resolve(),
            ROOT, FACTORLAB_ROOT, str(s["expected_factorlab_commit"]),
        )
        print(json.dumps({"status": result["status"], "new_fits": result["new_fits"]}, ensure_ascii=False)); return 0
